...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, G_list, accuracys, features='default', normalize=True,
                 assign_feat='default', max_num_nodes=0, feat_dim=16):
        self.accuracy_all = accuracys
        self.adj_all = []
        self.len_all = []
        self.feature_all = []
        self.label_all = []
        ...
        for G in G_list:
            ...
            self.set_feature_all(features, G, adj)
    ...
    def set_feature_all(self, features, G, adj):
        if features == 'default':
            f = np.zeros((self.max_num_nodes, self.feat_dim), dtype=float)
            for i, u in enumerate(G.nodes()):
                f[i, :] = util.node_dict(G)[u]['feat'].reshape(-1)
    ...
    def __getitem__(self, idx):
        ...
        return {'adj': adj_padded,
                ...
                'accuracy': self.accuracy_all[idx],
                'num_nodes': num_nodes,
