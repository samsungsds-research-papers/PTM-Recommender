...
from ptm_recommender.graph_models.gcnn.mlp import MLP
...
class GraphCNN(nn.Module):
        ...
        for layer in range(num_layers):
            if layer == 0:
                ...
                self.linears_prediction.append(
                    MLP(2, input_dim, int(0.5 * (input_dim + output_dim)), output_dim))
            else:
                # self.linears_prediction.append(nn.Linear(hidden_dim, output_dim))
                self.linears_prediction.append(
                    MLP(2, hidden_dim, int(0.5 * (hidden_dim + output_dim)), output_dim))
        self.sigmoid = nn.Sigmoid()
        self.pooling_eps = nn.Parameter(torch.zeros(self.num_layers - 1))
    ...
    def get_feature(self, batch_graph):
        X_concat = torch.cat(
        ...
        hidden_rep = self.get_hidden_rep(
            X_concat, padded_neighbor_list, Adj_block)
        ...
        for layer, h in enumerate(hidden_rep):
            if layer == 0:
                ...
                # pooled_h = pooled_h + (1. + self.pooling_eps[layer-1]) * torch.spmm(graph_pool, h)
                pooled_h += torch.spmm(graph_pool, h)
        score_over_layer += F.dropout(self.linears_prediction[len(hidden_rep) - 1](pooled_h),
                                      self.final_dropout,
                                      training=self.training)

        return score_over_layer, self.linears_prediction[len(hidden_rep) - 1](pooled_h), pooled_h

    def forward(self, batch_graph):
        score_over_layer, _, _ =self.get_feature(batch_graph)
        # print("score_over_layer shape : ", score_over_layer.shape)
        # print("batch_graph ", batch_graph)
        # assert False
        score = self.sigmoid(score_over_layer )
        return score

    def get_hidden_rep(self, X_concat, padded_neighbor_list=None, Adj_block=None):
        hidden_rep = [X_concat]
        h = X_concat
        for layer in range(self.num_layers - 1):
